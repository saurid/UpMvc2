...
<pre>
    ...
    {    
        ...
        return Up::database-&gt;fetchAll();
    }
</pre>

<p>I dina modeller har du åtkomst till databasen via ett gränssnitt mot PDO genom
service containern och <code>Up::database()</code> och metoderna <code>prepare(), execute(), fetchAll()</code>
...
<p><code>Up::database-&gt;prepare()</code> tar ett argument, en sträng med din SQL-sats. Vid en enkel
...
<p><code>Up::database-&gt;execute()</code> tar ett argument, en array med placeholders.
...
<p><code>Up::database-&gt;fetchAll()</code> använder vi till sist för att returnera
...
<pre>
    ...
    {    
        ...
        Up::database-&gt;execute(array(&apos;:id&apos; => $id));
...
Up::database-&gt;prepare(&apos;SELECT * FROM post&apos;);
Up::database-&gt;execute();
return Up::database-&gt;fetchAll();
...
<pre>
&lt;?php
<span class="comment">// Fil: Documentation/Model/Lipsum.php</span>
...
<p>Observera att denna exempelmodellen (och denna manual) ligger i mappen <code>Documentation/Model</code> och inte
